...
URL = "https://dre-granja-bot.onrender.com" # Troca se sua URL for diferente
...
async def producao(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        ws.append_row([
            agora, int(ovos), int(mort), float(racao),
            float(receita), float(custos), float(desp), obs
        ])

        await update.message.reply_text(f"✅ Lançado! Ovos: {ovos} | Mortes: {mort}")

    except Exception as e:
        await update.message.reply_text(f"❌ Erro ao lançar: {e}")
...
def main():
    ...
    # Sobe o servidor webhook. PTB já faz tudo.
    app.run_webhook(
        listen="0.0.0.0",
